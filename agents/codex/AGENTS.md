...
Create files lazily — only when there's something to write. This is the same knowledge base that Claude and other tools read from, so it's how context follows the work across tools.
